...
- [x] Define a finite-observation predicate adapter using an explicit manifest
...
- [ ] **Moved to 021** (needs asgard-lean 024 bound to the circuit): Define a separate evaluated-field adapter using Asgard/024's finite-support
...
- [x] Prove the adapters sound with reusable Lean theorems. A leaf checker success
...
- [x] Demonstrate coefficient constraints through a second derivative and the
...
- [ ] **Moved to 021** (needs asgard-lean 024 bound to the circuit): For fixed rational `a,c,R,T ≥ 0`, certify the actual heat solution with
...
- [ ] **Moved to 021** (needs asgard-lean 024 bound to the circuit): Check the concrete case `a=1,c=0,R=1,T=1`: bound 3 succeeds and bound 2
...
- [x] Negative tests catch basis/axis/manifest swaps, an altered initial profile,
...
- [x] Build all examples in `lake build`; audit new declarations against only
